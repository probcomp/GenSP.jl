...
function random_weighted(c::CustomImportance, target)
    ...
    particles = [simulate(c.q, ()) for _ in 1:c.num_particles] # TODO: what arguments should c.q take?
    # Compute weights
    target_scores = [Gen.assess(target.p, target.args, merge(target.constraints, get_choices(p)))[1] for p in particles]
    ...
    return get_choices(selected_particle), target_scores[selected_particle_index] - average_weight
end

function estimate_logpdf(c::CustomImportance, choices, target)
    # Generate k-1 particles
    unchosen = [Gen.simulate(c.q, ()) for _ in 1:c.num_particles-1]
    # Retained proposal trace
    push!(unchosen, generate(c.q, (), choices)[1])
    # Compute weights
    target_scores = [Gen.assess(target.p, target.args, merge(target.constraints, get_choices(p)))[1] for p in unchosen]
