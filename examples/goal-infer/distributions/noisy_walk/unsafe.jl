struct NoisyWalkUnsafe <: SPDistribution{ChoiceMap} end

const noisy_walk_unsafe = NoisyWalkUnsafe()

function GenSP.random_weighted(::NoisyWalkUnsafe, start, dest, speed, noise, scene, planner_params, num_ticks, dt)
...
function GenSP.estimate_logpdf(::NoisyWalkUnsafe, meas, start, dest, speed, noise, scene, planner_params, num_ticks, dt)
    maybe_path = plan_path(start, dest, scene, planner_params)
    planning_failed = maybe_path === nothing
