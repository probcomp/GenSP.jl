...
@gen function random_point_in_scene(scene::Scene)
    x ~ uniform(scene.xmin, scene.xmax)
    y ~ uniform(scene.ymin, scene.ymax)
    return Point(x, y)
end
...
@gen function generate_rrt(scene::Scene, init::Point, iters::Int, dt::Float64)
    tree = RRT(init) # init is the root of tree
    for iter=1:iters
        rand_conf::Point = {iter} ~ random_point_in_scene(scene)
...
@gen function plan_path(start::Point, dest::Point, scene::Scene, params::PlannerParams)
